 ...
 * 
 * @author Sergio Fernandez <sergio.fernandez@salzburgresearch.at>
 *
 */
public class AddStanbolServiceCommand extends Command {
    ...
    /**
     * Processes the pre-registration of an instance of Apache Stanbol, returning
     * the list of suitable reconciliation services available
     * 
     */
    @Override
    ...
    private Set<ReconciliationStanbolSite> retrieveReconciliations(String uri)
            throws IOException, JsonParseException {
        HttpEntity entity = HttpUtils.get(uri, JSON).getEntity();
    ...
    private ReconciliationStanbolSite buildReconciliation(String site)
            throws IOException {
        HttpEntity entity = HttpUtils.get(site, RDFXML).getEntity();

        Repository repo = new SailRepository(new MemoryStore());
        repo.initialize();

        Model model = Rio.parse(entity.getContent(), site, RDFFormat.RDFXML);
        RDFParser rdfParser = Rio.createParser(RDFFormat.RDFXML);
        rdfParser.setRDFHandler(new StatementCollector(model));

        RepositoryConfig repositoryConfig = RepositoryConfig.create(model, null);
        ...
        String query = "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> "
                ...
                + "SELECT ?name ?local " + "WHERE { " + "  <" + site + "> rdfs:label ?name ; "
        ...
        try (TupleQueryResult result = tupleQuery.evaluate()) {
            ...
            if (result.hasNext()) {
                BindingSet qs = result.next();
                ReconciliationStanbolSite reconciliation = new ReconciliationStanbolSite();
                reconciliation.setUri(site + RECONCILE_SUFFIX);
                reconciliation.setName(qs.getValue("name").stringValue());
                reconciliation.setLocal("true".equalsIgnoreCase(qs.getValue("local").stringValue()));
    ...
    private void serializeReconciliations(HttpServletResponse res, Set<ReconciliationStanbolSite> reconciliations)
            throws IOException {
