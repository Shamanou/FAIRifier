...
NewPrefixWidget.prototype.show = function(msg, def_prefix, onDone) {
    ...
    if (fetchOption === 'file') {
      ...
      self._elmts.file_upload_form.ajaxSubmit({
        ...
        success: function(format) {
          ...
          if (!format['RDFFormat']) {
            dismissBusy();
            alert('file format could not be detected, turtle, RDFXML, N-Tripples, triG, triX, RDFJSON, JSON-LD and N-Quads are supported.')
            return;
          } else {
            $.ajax({
  ...
  self._elmts.fetching_options_table.hide().find('input[name="vocab_fetch_method"]').click(function() {
...
NewPrefixWidget.prototype.suggestUri = function(prefix) {
  var self = this;
  $.get("command/rdf-extension/get-prefix-cc-uri", {
    prefix: prefix
  }, function(data) {
