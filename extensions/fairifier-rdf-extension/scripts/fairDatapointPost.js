...
fairDataPointPostDialog.prototype._constructBody = function(body) {
	...
	css.type = "text/css";
	css.innerHTML = "" + '.progress {' + 'border: 16px solid #f3f3f3; /* Light grey */' + 'border-top: 16px solid #3498db; /* Blue */' + 'border-radius: 50%;' + 'width: 120px;' + 'height: 120px;' + 'animation: spin 2s linear infinite;' + '}' +
	...
	var self = this;
	$('<p>' + 'The created RDF schema provided can now be uploaded to a FAIR Data Point. ' + '</p>').appendTo(body);
...
getFairCatalogs = function(rootUrl, self) {
	$.post('command/rdf-extension/get-fdp-info', {
	"uri" : rootUrl,
	"layer" : "catalog"
	}, function(data) {
