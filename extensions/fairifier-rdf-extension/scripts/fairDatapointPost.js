...
fairDataPointPostDialog.prototype._createDialog = function() {
  ...
  var header = $('<div></div>').addClass("dialog-header").text(
          "POST to Fair Data Point").appendTo(frame);
...
fairDataPointPostDialog.prototype._constructFooter = function(footer) {
  ...
  $(
          '<div style="hposition:absolute;visibility:visible;margin-left: 100px "></div>')
          .addClass("progress").hide().appendTo(footer);
...
fairDataPointPostDialog.prototype._replaceBaseUri = function(newBaseUri,
        doNotSave) {
  var self = this;
  var frame = DialogSystem.createDialog();
  if (doNotSave) {
...
getFairCatalogs = function(rootUrl, self) {
  $
          .post(
                  'command/rdf-extension/get-fdp-info',
                  {
                    "uri": rootUrl,
                    "layer": "catalog"
                  },
                  function(data) {
                    ...
                    var add_cat_html = $(
                            '<p><a href="#" bind="addCatalog">+ </a><span>add catalog</span></p>')
                            .appendTo(self._catalogDiv);
                    ...
                    data.content.forEach(function(element) {
                      $('<option></option>').attr('value',
                              element.uri.namespace + element.uri.localName)
                              .text(
                                      element.uri.localName + " - "
                                              + element.title.label).appendTo(
                                      add_cat_available_html);
                    ...
                    elmts.addCatalog
                            .click(function(evt) {
                              evt.preventDefault();
                              fairDataPointPostCatalogDialog = new FairDataPointPostCatalogDialog(
                                      function(catalog) {
                                        $('<option></option>')
                                                .attr(
                                                        'value',
                                                        catalog['http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier'].url)
                                                .prop('selected', true)
                                                .text(
                                                        catalog['http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier']
                                                                + " - "
                                                                + catalog['http://purl.org/dc/terms/title'])
                                                .appendTo(
                                                        add_cat_available_html);
                                        self._datasetDiv.html('');
                                        self._distributionDiv.html('');
                    ...
                    add_cat_available_html
                            .click(
                                    function(e) {
                                                ...
                                                .forEach(function(element) {
                                                  if (e.target.value == (element.uri.namespace + element.uri.localName)) {
                                                    self.fairDataPointPost.catalog = {
                                                      'http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier': element.uri.namespace
                                                              + element.uri.localName,
                                      ...
                                      getFairDatasets(
                                              $(
                                                      'select.catalogs option:selected')
                                                      .val(), self);
                                    }).change();
  ...
  $
          .post(
                  ...
                  function(data) {
                    ...
                    var add_dat_html = $(
                            '<p><a href="#" bind="addDataset">+ </a><span>add dataset</span></p>')
                            .appendTo(self._datasetDiv);
                    ...
                    elmts.addDataset
                            .click(function(evt) {
                              evt.preventDefault();
                              fairDataPointPostDatasetDialog = new FairDataPointPostDatasetDialog(
                                      function(dataset) {
                                        dataset['http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier'] = self.fairDataPointPost.baseUri
                                                + "/dataset/"
                                                + dataset['http://purl.org/dc/terms/title']
                                                        .replace(" ", "_")
                                                + "_"
                                                + dataset["http://purl.org/dc/terms/hasVersion"]
                                                        .replace(" ", "_");
                                        $('<option></option>')
                                                .attr(
                                                        'value',
                                                        dataset['http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier'].url)
                                                .prop('selected', true)
                                                .text(
                                                        dataset['http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier'].url
                                                                + " - "
                                                ...
                                                .forEach(function(element) {
                                                  if (e.target.value == (element.uri.namespace + element.uri.localName)) {
                                                    self.fairDataPointPost.dataset = {
                                                      ...
                                                      // element.datasetIssued,
                                                      // "http://purl.org/dc/terms/language":
                                                      // element.language,
                                                      // "http://purl.org/dc/terms/license":
                                                      // element.license,
                                                      // "http://purl.org/dc/terms/modified":
                                                      // element.datasetModified,
...
addFairDistribution = function(self) {
  $
          .get(
                  "command/rdf-extension/get-push-configuration",
                  function(conf) {
                    conf = JSON.parse(conf.data).xml;
                    if (conf.pushToFtp.enabled && !($(".ftpRadio").length)) {
                      var ftp_html = $(
                              '<br><br><input type="radio" value="ftp" class="ftpRadio" bind="ftp"><span>push FAIRified data to FTP</span><br>')
                              .appendTo(self._datasetDiv);
                      var ftp_elmts = DOM.bind(ftp_html);
                    }
                    if (conf.pushToVirtuoso.enabled
                            && !($(".virtuosoRadio").length)) {
                      var virtuoso_html = $(
                              '<br><br><input type="radio" value="virtuoso" class="virtuosoRadio" bind="virtuoso"><span>push FAIRified data to tripple store</span><br>')
                              .appendTo(self._datasetDiv);
                    ...
                    $(".ftpRadio")
                            .click(
                                    function() {
                                      ...
                                      $('input.virtuosoRadio').removeAttr(
                                              'checked');
                                    });
                    $(".virtuosoRadio")
                            .click(
  ...
  var add_dist_html = $(
          '<p><a href="#" bind="addDistribution">+ </a><span>add distribution</span><br><span id="distribution" bind="distribution"></span></p>')
          .appendTo(self._distributionDiv);
  var elmts = DOM.bind(add_dist_html);
  elmts.addDistribution
          .click(function(evt) {
            evt.preventDefault();
            fairDataPointPostDistributionDialog = new FairDataPointPostDistributionDialog(
                    function(distribution) {
                      self.fairDataPointPost.distribution = distribution;
                      fairDataPointPost = self.fairDataPointPost;
                      $("#distribution")
                              .text(
                                      self.fairDataPointPost.distribution['http://rdf.biosemantics.org/ontologies/fdp-o#metadataIdentifier'].url
                                              + " - "
                                              + self.fairDataPointPost.distribution['http://purl.org/dc/terms/title']);
                    });
          });
