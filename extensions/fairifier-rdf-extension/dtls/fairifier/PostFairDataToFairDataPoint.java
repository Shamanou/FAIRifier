...
public class PostFairDataToFairDataPoint extends Command {
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      if (!catalog.getBoolean("_exists")) {
        ...
        catalogPost =
            pushMetadataToFdp(uuid_catalog, catalogMetadata, "catalog", fdp.getString("baseUri"));
      }
      ...
      if (!dataset.getBoolean("_exists")) {
        ...
        datasetPost =
            pushMetadataToFdp(uuid_dataset, datasetMetadata, "dataset", fdp.getString("baseUri"));
      }
  ...
  private DatasetMetadata addPropertiesToDataset(JSONObject dataset, String uuid_dataset,
    ...
    if (dataset.has(LANGUAGEPREDICATE) && (dataset.getJSONArray(LANGUAGEPREDICATE) != null)) {
      datasetMetadata
          .setLanguage(FACTORY.createIRI(dataset.getJSONArray(LANGUAGEPREDICATE).getString(0)));
    }
    ...
    if (catalog.getBoolean("_exists")) {
      datasetMetadata
          .setParentURI(FACTORY.createIRI(catalog.getString(METADATAIDENTIFIERPREDICATE)));
    } else {
  ...
  private DistributionMetadata addPropertiesToDistribution(JSONObject distribution,
      ...
      JSONObject uploadConfiguration, String data) throws JSONException, InstantiationException,
    ...
    DistributionMetadata distributionMetadata = getMetadata(DistributionMetadata.class,
        distribution, fdp.getString("baseUri") + "/distribution/" + name + "_" + uuid_distribution);
    ...
    if (fdp.getString("uploadtype").equals("ftp")) {
      distributionMetadata
          .setDownloadURL(FACTORY.createIRI("ftp://" + uploadConfiguration.getString("host")
              + uploadConfiguration.getString("directory") + name + ".ttl"));
    } else if (fdp.getString("uploadtype").equals("virtuoso")) {
    ...
    if (dataset.getBoolean("_exists")) {
      distributionMetadata
          .setParentURI(FACTORY.createIRI(dataset.getString(METADATAIDENTIFIERPREDICATE)));
    } else {
      name = dataset.getString(TITLEPREDICATE).replace(" ", "_") + "_"
          + dataset.getString(VERSIONPREDICATE).replace(" ", "_");
    ...
    if (distribution.has(DESCRIPTIONPREDICATE)) {
      distributionMetadata
          .setDescription(FACTORY.createLiteral(distribution.getString(DESCRIPTIONPREDICATE)));
    }
  ...
  private String pushMetadataToFdp(String uuid, Metadata metadata, String metadataType,
    ...
    String metadataId =
        baseUri + "/" + metadataType + "?id=" + metadata.getTitle().getLabel().replace(" ", "_")
